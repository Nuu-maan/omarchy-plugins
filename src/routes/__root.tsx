...
export const Route = createRootRoute({
  ...
  component: Root,
  notFoundComponent: () => <section className="page-intro"><p className="eyebrow">404 / not found</p><h1>Nothing at this address.</h1><p>The plugin may have moved or been suspended.</p><Link className="button primary" to="/">Browse plugins</Link></section>,
  ...
  return <html lang="en" data-theme="matte" suppressHydrationWarning>
    ...
    <body>
      ...
      <header className="site-header"><div className="header-inner">
        <Link to="/" className="brand" aria-label="Omarchy Plugins home"><Blocks size={23} aria-hidden="true" /><span>omarchy<span className="brand-suffix"> / plugins</span></span></Link>
        <nav aria-label="Main navigation">
          <Link to="/" activeOptions={{ exact: true }} activeProps={{ 'aria-current': 'page' }}>Discover</Link>
          <Link to="/activity" activeProps={{ 'aria-current': 'page' }}>Releases</Link>
          <Link to="/security" activeProps={{ 'aria-current': 'page' }}>Trust</Link>
        </nav>
        <div className="header-actions"><ThemePicker /><a href={repositoryUrl} className="icon-button github-link" aria-label="Registry on GitHub"><Github size={19} aria-hidden="true" /></a><Link to="/publish" className="button primary header-publish">Publish <ArrowUpRight size={16} aria-hidden="true" /></Link></div>
      </div></header>
      <main id="main" className="site-main" tabIndex={-1}><Outlet /></main>
      <footer className="site-footer"><div className="footer-top"><Link to="/" className="brand"><Blocks size={21} aria-hidden="true" />omarchy / plugins</Link><span>Built by the community. Open to everyone.</span></div><div className="footer-bottom"><p>Independent registry. Not affiliated with Omarchy.</p><div><a href="/registry.json">Registry JSON</a><a href={repositoryUrl}>Source</a><Link to="/security">Security</Link><a href="https://omarchy.org/">Omarchy ↗</a></div></div></footer>
      <Scripts />
    </body>
  </html>
}
